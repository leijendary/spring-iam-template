...
public class RegisterVerifyControllerV1 {
    ...
    public CompletableFuture<DataResponse<RegisterVerifyResponseV1>> verify(
            @Valid @RequestBody final VerifyRequestV1 request) {
        final var verifyResponse = registerVerifyFlow.verifyV1(request);
        final var response = DataResponse.<RegisterVerifyResponseV1>builder()
                .data(verifyResponse)
                .object(RegisterVerifyResponseV1.class)
                .build();
